...
function WorldMap({ byCountry }) {
  const countMap = Object.fromEntries((byCountry || []).map(c => [c.country_code, c.count]))
              ...
              geographies.map(geo => {
                ...
                return (
                  <Geography key={geo.rsmKey} geography={geo}
                    fill={count > 0 ? `rgba(99,102,241,${intensity})` : '#E5E7EB'}
                    stroke="#fff" strokeWidth={0.3}
                    style={{ default: { outline: 'none' }, hover: { fill: '#4F46E5', outline: 'none' }, pressed: { outline: 'none' } }}
                  />
                )
              })
            }
