...
The generated PDF is used as the source document for the entire Q&A pipeline
and contains original, educational content (no real exam questions).
"""
...
def create_sample_pdf(output_path: Path | str) -> Path:
    """
    Generates a multi-chapter sample PDF document and saves it to *output_path*.
    ...
    story.append(
        Paragraph(
            "This document is intended for educational purposes and serves as the source "
            "material for an AI-powered Q&amp;A generation pipeline.",
            ParagraphStyle(
                "Disclaimer",
    ...
    doc.build(story)
    logger.info("Sample PDF created at: %s", output_path.resolve())
    return output_path.resolve()


# ── Backward-compat re-export ─────────────────────────────────────────────────
# The canonical implementation with observability now lives in
# src/ingestion/pdf_generator.py.  This re-export ensures that any code
# importing `from src.pdf_generator import create_sample_pdf` gets the new
# version (with structured logging) rather than the original above.
from src.ingestion.pdf_generator import create_sample_pdf  # noqa: F811, E402
...
if __name__ == "__main__":
    ...
    print(f"PDF saved to: {SAMPLE_PDF_PATH}")
